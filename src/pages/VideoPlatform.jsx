...
const CityCard = ({ city }) => {
  ...
  return (
    <Link
      to={city.contact}
      className="bg-white rounded-md shadow w-[270px] h-[300px] overflow-hidden mx-4 hover:shadow-lg hover:scale-105 transition-transform flex-shrink-0 snap-center"
    >
      <img
        src={city.imageURL}
        className="h-[150px] w-full object-cover cursor-pointer"
...
const IndustryCard = ({ industry }) => {
  const navigate = useNavigate();
  return (
    <div className="border-[0.5px] border-[#003DA5] bg-white rounded-md shadow w-[270px] h-[300px] overflow-hidden hover:shadow-lg hover:scale-105 transition-transform mx-5 flex-shrink-0 snap-center">
      <img
        src={industry.imageURL}
        className="h-[150px] w-full object-cover cursor-pointer"
        alt={industry.name}
...
const VideoCard = ({ video, toggleFavorite, openResumeModal, deleteVideo, onEdit }) => {
  return (
    <div className="m-6 w-[300px] h-[450px] border-[0.5px] border-[#003DA5] rounded-lg shadow-md relative bg-white flex-shrink-0 snap-center">
      <video controls className="w-full h-[250px]">
        <source src={video.videoURL} type="video/mp4" />
        Your browser does not support the video tag.
      </video>
...
const VideoPlatform = () => {
  ...
  const deleteVideo = async (id) => {
    try {
      ...
      setVideos((prev) => prev.filter((video) => video.id !== id));
      alert(`Video with id deleted successfully.`);
    } catch (error) {
      console.error(`Error deleting video with id ${id}:`, error);
    }
  };
...
const videoSearchRef = useRef(null);
  ...
  useEffect(() => {
    const updateScrollClass = (ref, items, cardWidth) => {
      ...
      const container = ref.current;
      const contentWidth = items.length * cardWidth;
      const sectionWidth = 0.8 * window.innerWidth;
      if (contentWidth > sectionWidth) {
  ...
  return (
    <div className="font-['Crimson_Pro']">
      ...
      {/* Regions Section */}
      <section className="bg-gray-100 py-8 px-4 sm:px-10">
        <div className="max-w-7xl mx-auto text-center">
          ...
          {Object.entries(groupedByState).map(([state, cities]) => (
            <div key={state} className="mb-8">
              ...
              <div
                ref={regionScrollRef}
                className="flex justify-center flex-nowrap overflow-x-auto scroll-smooth scrollbar-thin scrollbar-track-gray-100 scrollbar-thumb-[#003DA5] snap-x snap-mandatory min-h-[320px]"
              >
          ...
          {filteredRegions.length === 0 && (
            <p className="text-center text-gray-500 mt-8 w-full">No regions found.</p>
          )}
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            {filteredIndustries.length > 0 ? (
              ...
              <p className="text-center text-gray-500 mt-8 w-full">No industries found.</p>
            )}
          </div>
        </div>
      </section>
      ...
      <section className="bg-gray-100 py-8 px-4 sm:px-10">
        <div className="max-w-7xl mx-auto">
          ...
          <div
            ref={videoScrollRef}
            className="flex justify-center flex-nowrap overflow-x-auto scroll-smooth scrollbar-thin scrollbar-track-gray-100 scrollbar-thumb-[#003DA5] snap-x snap-mandatory min-h-[470px]"
          >
            {filteredVideos.length > 0 ? (
              filteredVideos.map((video, index) => (
                <VideoCard
              ...
              <p className="text-center text-gray-500 mt-8 w-full">No candidates found.</p>
            )}
          </div>
        </div>
      </section>
